 ...
 * 图片加载类
 *
 * @author qcj 类说明，本地图片管理器，
 *         <p/>
 *         代表某文件的id这个类对外的开放的最重要的两个方法就是displayImage(ImageView imageView, String
 *         url, int resId)
 *         <p/>
 *         displayImage(ImageView imageView, String url, int resId, int width,
 *         int height)
 */
public class LocalImageManager {

    private static LocalImageManager imageManager;
     ...
     * @return
     */
    public static LocalImageManager from(Context context) {
        ...
        if (imageManager == null) {
            imageManager = new LocalImageManager(myapp);
        }
            ...
            switch (msg.what) {
                ...
                case MSG_REQUEST: // 收到请求
                    ...
                    if (msg.obj != null && msg.obj instanceof ImageRef) {
                        ...
                        int mBitmapSize = option.outHeight * option.outWidth * 4;
                        option.inSampleSize = mBitmapSize / (1000 * 2000);
                        option.inJustDecodeBounds = false;
                        tBitmap = BitmapFactory.decodeFile(url, option);
