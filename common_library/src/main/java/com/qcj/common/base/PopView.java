...
import com.qcj.common.interf.PopInterface;
...
public abstract class PopView implements PopInterface {
    ...
    public PopResultListener mResultListener;
     ...
     * @return
     */
    public boolean isSetFocuseable() {
        return true;
    }
     ...
     * @param y
     */
    public void shopPopByDector(int x, int y) {
        if (mActivity != null) {
            mPopWindow.setOutsideTouchable(true);
            View parent = mActivity.getWindow().getDecorView();//高度为手机实际的像素高度
            mPopWindow.showAtLocation(parent, Gravity.NO_GRAVITY, x, y);
        }
    }
    ...
    public interface PopResultListener {
         ...
         * @return
         */
        Object onPopResult(Object object,int wtf);
    }
